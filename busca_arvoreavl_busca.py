...
import random
import os
from algoritimos.arvore_avl import ArvoreAVL
...
for arquivo in arquivos:
    ...
    arvore_avl.comparacoes_totais = 0

    # Preenche a árvore AVL
    for x in lista:
        arvore_avl.inserir(x)

    raiz, esquerda, direita = arvore_avl.ver_lados()

    print("Raiz:", raiz)
    print("Nós da esquerda:", len(esquerda))
    print("Nós da direita:", len(direita))

    #print(arvore_avl.fator_balanceamento_atualizado)
    ...
    for x in casos_piores:
        ...
        arvore_avl.buscar(x)
        total_comparacoes_pior.append(arvore_avl.comparacoes_totais)
        ...
        #print(f"Comparações Pior Cenário : {arvore_avl.comparacoes_totais}")
        arvore_avl.comparacoes_totais = 0
    ...
    for x in numeros_aleatorios:
        ...
        arvore_avl.buscar(x)
        total_comparacoes_aleatorio.append(arvore_avl.comparacoes_totais)
        ...
        #print(f"Comparações Aleatório {arvore_avl.comparacoes_totais}")
        arvore_avl.comparacoes_totais = 0
    ...
    nome_arquivo_csv = os.path.join(nome_diretorio, 'busca_arvoreavl_nao_ordenada.csv')
